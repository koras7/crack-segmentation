    ...
    def _solve_rhs_with_known_y(self, y: torch.Tensor) -> torch.Tensor:
        ...
        def integral(x):
            ...
            if self.spatial_integration is False:
                ...
                # print("samp:", self.samp)
                # print("volume:", self.volume)
                # print("dim:", self.dim)
                # print("Expected total elements:", self.n_batch * self.samp * self.volume * self.dim)
                # print("Actual total elements:", y_.numel())

                                  
                
                y_in = y_.view([self.n_batch,self.samp*self.volume,self.dim])
                # y_in = y_.reshape(self.n_batch, -1, self.dim)

                # batch_size, num_points, _ = y_in.shape

                # space = self.spatial_domain
                # space = space.view(1, 1, -1)   # flatten spatial domain
                # space = space.repeat(batch_size, num_points, 1)  # match batch and num_points

                
                
                y_in = torch.cat([y_in,space,time.repeat(self.n_batch,self.volume,1).sort(dim=-2)[0]],-1)
                #y_in = torch.cat([y_in, space, time.repeat(batch_size, num_points, 1).sort(dim=-2)[0]], -1)
