...
#     def __init__(self, Data, masks, augment=False):
#         self.Data = Data.float()
#         self.masks = masks.float()
#         self.augment = augment

#         # ✅ Define augmentations only if requested
#         if self.augment:
...
#                 A.ElasticTransform(alpha=30, sigma=6, alpha_affine=5, p=0.25),
...
#     def __getitem__(self, index):
#         image = self.Data[index]   # [256, 256, 1, 3]
#         mask = self.masks[index]   # [256, 256]

#         # Convert from torch → numpy
#         image_np = image.cpu().numpy()
#         mask_np = mask.cpu().numpy()

#         #  Squeeze time dimension → [H, W, C]
#         if image_np.ndim == 4:  # [H, W, 1, 3]
#             image_np = image_np.squeeze(2)  # → [H, W, 3]
...
#         image_t = torch.from_numpy(image_np).float()
#         mask_t = torch.from_numpy(mask_np.squeeze(-1)).long()

#         # Reinsert time dim → [H, W, 1, C]
#         image_t = image_t.unsqueeze(2)
...
class Segmentation_Dataset(torch.utils.data.Dataset):
    def __init__(self, Data, masks, augment=False, device="cuda"):
        self.device = device
        self.Data = Data.float().to(device)      # store on GPU
        self.masks = masks.float().to(device)    # store on GPU
        self.augment = augment
    ...
    def __getitem__(self, index):
        ...
        # move temporarily to CPU for Albumentations
        image_np = image.cpu().numpy()
        mask_np = mask.cpu().numpy()
        ...
        # back to torch
        image_t = torch.from_numpy(image_np).float().unsqueeze(2).to(self.device)
        mask_t = torch.from_numpy(mask_np.squeeze(-1)).long().to(self.device)
